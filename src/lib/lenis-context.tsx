...
export const LenisProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const scrollTo = (target: string | HTMLElement, options?: LenisScrollOptions) => {
    const resolvedTarget = typeof target === "string" ? normalizeStringTarget(target) : target;

    const isSectionTarget = typeof resolvedTarget === "string" && resolvedTarget.startsWith("#");
  ...
  useEffect(() => {
    ...
    const tick = () => {
      const el = typeof pending.target === "string" ? document.querySelector(pending.target) : pending.target;
