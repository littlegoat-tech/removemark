...
const tableVariants = cva(
  ["relative border border-border bg-card", "rounded-lg shadow-md card-highlight", "not-prose"],
  {
    variants: {
      density: {
        comfortable: "text-sm",
...
const rowVariants = cva(["relative grid items-center", "border-b border-border/60 last:border-0"], {
  variants: {
    selected: {
      true: "bg-card-muted",
      false: "",
...
export interface TableProps<TData> extends React.HTMLAttributes<HTMLDivElement>, VariantProps<typeof tableVariants> {
...
export function Table<TData>({
  ...
  const rowSelection = isRowSelectionControlled ? rowSelectionState : internalRowSelection;
  ...
  React.useEffect(() => {
    ...
    const selectedRows = table.getSelectedRowModel().rows.map((tableRow) => tableRow);
  ...
  React.useEffect(() => {
    ...
    const isActiveRowPresent = rows.some((row) => resolveCompositeId(row.id) === activeCompositeId);
  ...
  return (
    <div ref={tableRef} className={cn(tableVariants({ density }), className)} {...props}>
        ...
        >
          {headerGroup.headers.map((header) => {
            ...
            const sortStateVariant: "asc" | "desc" | "none" =
              sortStateRaw === "asc" || sortStateRaw === "desc" ? sortStateRaw : "none";
            ...
            return (
              ...
              <button
                ...
                aria-sort={sortStateRaw === "asc" ? "ascending" : sortStateRaw === "desc" ? "descending" : "none"}
                className={cn(
                  headerCellVariants({
                    align,
                    sortable: true,
              ...
              >
                ...
                <span className="ml-1 flex items-center justify-center">
                  <Icons.CaretSort aria-hidden="true" className="h-3.5 w-3.5 text-foreground/70" />
                </span>
              </button>
            );
          })}
      ...
      >
        {rows.length === 0 && (
          <div className="px-6 py-10 text-center text-foreground/70">{emptyState ?? "No data available"}</div>
        )}
        ...
        {rows.map((row) => {
          ...
          return (
            <Ariakit.CompositeItem
              ...
              render={(itemProps) => (
                <div {...itemProps} style={{ ...itemProps.style, gridTemplateColumns }}>
