...
const textareaVariants = cva(
  ["h-full min-w-full bg-transparent outline-none", "placeholder:text-foreground/45", "disabled:cursor-not-allowed"],
  {
    variants: {
      size: {
...
export const Textarea: React.FC<TextareaProps> = ({
  ...
  return (
    <div className={cn("not-prose", containerClassName)}>
      ...
      {displayErrorMessage && (
        <p id={errorId} className={messageVariants({ type: "error" })} aria-live="polite">
          {errorMessage}
        </p>
      )}
      {displaySuccessMessage && (
        <p id={successId} className={messageVariants({ type: "success" })} aria-live="polite">
          {successMessage}
        </p>
      )}
