  ...
  ],
  {
    variants: {
      variant: {
        ...
        error: ["border-destructive/40", "focus-within:ring-destructive/50 dark:focus-within:ring-destructive/50"],
        success: ["border-success/40", "focus-within:ring-success/50 dark:focus-within:ring-success/50"],
      },
      size: {
        sm: "h-8",
        md: "h-9",
...
const inputVariants = cva(
  ["w-full bg-transparent outline-none", "placeholder:text-foreground/45", "disabled:cursor-not-allowed"],
  {
    variants: {
      size: {
...
const iconVariants = cva(["absolute top-1/2 -translate-y-1/2 transform", "text-foreground/45"], {
  variants: {
    position: {
      left: "left-3",
      right: "right-3",
  ...
  return (
    <div className={`${containerClassName}`}>
      ...
      <div className={cn(wrapperVariants({ variant, size }), wrapperClassName)}>
        ...
        <input
          className={cn(inputVariants({ size, hasLeftIcon, hasRightIcon }), className)}
          id={inputId}
          disabled={disabled}
      ...
      {displayErrorMessage && (
        <p id={`${inputId}-error`} className={messageVariants({ type: "error" })}>
          {errorMessage}
        </p>
      )}

      {!displayErrorMessage && successMessage && (
        <p id={`${inputId}-success`} className={messageVariants({ type: "success" })}>
          {successMessage}
        </p>
      )}

      {helperText && (
        <p id={`${inputId}-helper`} className={messageVariants({ type: "helper" })}>
          {helperText}
        </p>
      )}
    </div>
