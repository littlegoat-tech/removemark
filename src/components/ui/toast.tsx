...
const getDefaultSwipeDirections = (position?: ToastPosition | null): SwipeDirection[] => {
  if (!position) {
...
const ToastItem: React.FC<ToastItemProps> = React.memo(
  ({
    ...
    const [swipeDismissDirection, setSwipeDismissDirection] = useState<SwipeDirection | null>(null);
    ...
    const swipeDirections = useMemo(() => getDefaultSwipeDirections(position), [position]);
    ...
    const getFocusableElements = useCallback(() => {
      if (!toastRef.current) return [];
      return Array.from(toastRef.current.querySelectorAll(FOCUSABLE_SELECTORS)) as HTMLElement[];
    }, []);
    ...
    useEffect(() => {
      ...
      const isPaused = isGroupHovered || isItemHovered || isSwiping || hiddenByStacking;
      if (isPaused) {
        if (timeoutRef.current) {
          clearTimeout(timeoutRef.current);
          timeoutRef.current = null;
    ...
    }, [duration, shouldClose, handleClose, isGroupHovered, isItemHovered, isSwiping, hiddenByStacking]);
    ...
    const maxVisibleIndex = Math.max(0, ANIMATION_CONFIG.MAX_VISIBLE_TOASTS - 1);
    ...
    const visibleScale = Math.max(ANIMATION_CONFIG.MIN_SCALE, 1 - visibleIndex * ANIMATION_CONFIG.SCALE_FACTOR);
    const zIndex = ANIMATION_CONFIG.Z_INDEX_BASE - renderIndex;

    const transformStyle = useMemo(() => {
      const baseOffsetY = stackHidden ? resolvedHiddenCollapsedOffset : resolvedCollapsedOffset;
      const promotionOffset = typeof expandedGap === "number" ? expandedGap : ANIMATION_CONFIG.EXPANDED_GAP;
      ...
      let scaleValue = stackHidden ? (visibleIndex === 0 ? 1 : visibleScale) : isStackLeader ? 1 : scale;
    ...
    const swipeCursorClass = canSwipe ? (isSwiping ? "cursor-grabbing" : "cursor-grab") : undefined;
    ...
    const handlePointerUp = useCallback(
      (event: React.PointerEvent<HTMLDivElement>) => {
        ...
        const elapsed = dragStartTimeRef.current ? Date.now() - dragStartTimeRef.current : 0;
        ...
        if (axis) {
          ...
          const meetsThreshold = Math.abs(distance) >= SWIPE_DISMISS_THRESHOLD || velocity > SWIPE_DISMISS_VELOCITY;
        ...
        >
          <div className={cn(toastContentVariants({ variant }))}>
            ...
            <div className="p-4 pr-8">
              {title && (
                <div id={titleId} className="mb-1 text-sm leading-none font-medium select-none">
                  {title}
                </div>
              )}
              {description && (
                <div id={descriptionId} className="text-sm leading-snug text-foreground/70 text-balance select-none">
                  {description}
                </div>
              )}
              {action && (
...
const ToastManager: React.FC<{
  ...
  const toastsByPosition = useMemo(() => {
    ...
    Object.keys(grouped).forEach((position) => {
      ...
      grouped[positionKey] = list.map((toast, orderIndex) => {
        let stackIndex = activeIndexMap.get(toast.id) ?? previousStackIndexRef.current[toast.id];
  ...
  const positionEntries = useMemo(() => Object.entries(toastsByPosition), [toastsByPosition]);
  ...
  const collapsedOffsetData = useMemo(() => {
    ...
    for (const [pos, group] of positionEntries as [ToastPosition, PositionedToast[]][]) {
      ...
      for (let i = 0; i < activeToasts.length; i++) {
        ...
        if (prevHeight == null || currentHeight == null || Number.isNaN(prevHeight) || Number.isNaN(currentHeight)) {
          offsetsForActive.push(fallbackOffset);
          continue;
        }

        if (isTopPosition) {
          offsetsForActive.push(prevOffset + (prevHeight - currentHeight + ANIMATION_CONFIG.STACK_OFFSET));
        } else {
          offsetsForActive.push(prevOffset + (currentHeight - prevHeight - ANIMATION_CONFIG.STACK_OFFSET));
        }
      }
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      const changed = Object.keys(next as Record<string, boolean>).some(
        (k) => (next as Record<string, boolean>)[k] !== (hovered as Record<string, boolean>)[k],
      );
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-50">
      {positionEntries.map(([position, positionToasts]) => {
        ...
        const maxVisibleStackIndex = Math.min(Math.max(activeToasts.length - 1, 0), visibleStackLimit);
        ...
        const sharedHiddenCollapsedOffset =
          lastVisibleRenderIndex >= 0 ? collapsedOffsets?.[lastVisibleRenderIndex] : undefined;
        return (
          <React.Fragment key={position}>
            {positionToasts.map((toast, idx) => {
              const toastIsHidden = toast.index >= ANIMATION_CONFIG.MAX_VISIBLE_TOASTS;
