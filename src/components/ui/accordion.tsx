...
const accordionItemVariants = cva("transition-[box-shadow] ease-out-quad duration-100", {
  variants: {
    variant: {
...
const accordionContentVariants = cva("overflow-hidden transition-[height,opacity] duration-450 ease-smooth", {
  variants: {
    size: {
      sm: "text-xs",
      md: "text-sm",
...
type DisclosureButtonProps = React.ComponentPropsWithoutRef<typeof Ariakit.Disclosure>;
...
const AccordionStyleContext = createContext<AccordionStyleContextType | undefined>(undefined);
...
const AccordionBehaviorContext = createContext<AccordionBehaviorContextType | undefined>(undefined);
...
export const Accordion: React.FC<AccordionProps> = React.memo(
  ({ children, variant = "default", size = "md", className, type = "single", onValueChange, ...props }) => {
    ...
    const itemsRef = React.useRef<Map<string, Ariakit.DisclosureStore>>(new Map());

    const registerItem = useCallback((value: string, store: Ariakit.DisclosureStore) => {
...
export const AccordionItem: React.FC<AccordionItemProps> = React.memo(
  ({ children, value, id, className, disabled = false, defaultOpen = false, ...props }) => {
    const { variant } = useAccordionStyle();
    const { registerItem, unregisterItem, notifyItemOpen } = useAccordionBehavior();
    ...
    if (variant === "card") {
      return (
        <AccordionItemContext.Provider value={contextValue}>
          <div className={cn(accordionItemVariants({ variant, isOpen: open }), className)} {...props}>
            <div className="relative rounded-lg border border-border bg-card shadow-md card-highlight">{children}</div>
          </div>
...
export const AccordionContent: React.FC<AccordionContentProps> = React.memo(({ children, className, ...props }) => {
  const { variant, size } = useAccordionStyle();
  ...
  return (
    <div ref={contentRef} className={cn(accordionContentVariants({ size }), className)} style={contentStyle} {...props}>
      <section
        id={contentId}
