  ...
  return (
    ...
    >
      <div className="w-full flex flex-col gap-2 px-4 py-8 md:p-8">
        ...
        <h2 className="text-xl md:text-2xl text-foreground font-medium text-balance leading-none">{title}</h2>
        <p className="text-base text-foreground/70 font-medium text-balance leading-relaxed md:max-w-1/2">
          {description}
        </p>
      </div>
      <div className={`relative w-full h-full p-4 md:p-8 ${className ?? ""}`}>{children}</div>
    </section>
  );
}
